...
public class MessageProxyImpl implements MessageProxy {
    ...
    public MessageWrapper createMessageWapper(String sessionId, QiaoQiaoHua.Model qiaoqiaohua) {
        ...
        switch (cmd) {
            case QiaoqiaoConst.CommandLine.BIND: //首次连接时候生成SessionId
                String sessionId0 = UUID.randomUUID().toString(); //TODO
            ...
            case QiaoqiaoConst.CommandLine.MESSAGE:
                ...
                if (!StringUtils.isEmpty(receiver) && QiaoqiaoConst.MessageType.SEND == msgType) {  //消息接受人为用户
                    //根据消息接收人找到对应的sessionId 如果么有
                    List<String> sessionIds = sessionManager.getSessionIdsByAccount(receiver);
                    return new MessageWrapper(MessageWrapper.MessageProtocol.GROUP, sessionId, sessionIds, qiaoqiaohua);
                } else if (!StringUtils.isEmpty(groupId) && QiaoqiaoConst.MessageType.SEND == msgType) { //消息接收人为群组
                    return new MessageWrapper(MessageWrapper.MessageProtocol.GROUP, sessionId, "", qiaoqiaohua);
                } else if (QiaoqiaoConst.MessageType.SEND_REPLY == msgType) { //消息为应答服务器内容
                    return new MessageWrapper(MessageWrapper.MessageProtocol.SEND_REPLY, sessionId, "", qiaoqiaohua);
                } else if (QiaoqiaoConst.MessageType.NOTIFY == msgType) { //消息为通知消息
                    return null;
                } else if (QiaoqiaoConst.MessageType.NOTIFY_REPLY == msgType) { //消息为通知应答消息
                    return null;
                }
        }
